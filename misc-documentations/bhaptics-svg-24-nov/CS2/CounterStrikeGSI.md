...
# Valve CS:GO Game State Integration – Python & C Interop Notes

## Game State Integration Overview

Valve's [Game State Integration for CS:GO](https://developer.valvesoftware.com/wiki/Counter-Strike:_Global_Offensive_Game_State_Integration) exposes game state as JSON via HTTP POST to a local or remote endpoint. By placing a configuration file inside the game directory (`game/csgo/cfg`), the game will POST relevant state info (player status, map, round, bomb, weapons, etc.) to your service when changes occur.
...
- Full structure and options in the [official documentation](https://developer.valvesoftware.com/wiki/Counter-Strike:_Global_Offensive_Game_State_Integration).

## Capturing Game State (Language Agnostic)
1. Start a local HTTP server listening on the `uri` you specify (Python, NodeJS, C, etc.).
2. The game will POST JSON payloads with the current state.
3. Parse and act on these using your chosen language.
...
### C Example (Pseudocode/Pointer)
- Use a tiny HTTP library like [Mongoose](https://github.com/cesanta/mongoose) or [libmicrohttpd](https://www.gnu.org/software/libmicrohttpd/).

- C will receive the POST and parse the JSON (e.g. with [cJSON](https://github.com/DaveGamble/cJSON)).

- To expose C functionality to Python, options include:
  - **ctypes**: Expose C .dll/.so functions as Python-callable
  - **cffi**: More dynamic interface (especially if you have source)
  - **Custom Python extension**: Write a real Python module in C/C++
  - **IPC/Sockets**: Run C as its own process, talk via sockets or named pipes

#### Example: Calling C from Python via ctypes
```c
// c_gsi_handler.c
#include <stdio.h>
__declspec(dllexport) void process_gsi_payload(const char* json_payload) {
    printf("Received: %s\n", json_payload);
    // TODO: parse/process
}
```
```python
import ctypes
lib = ctypes.CDLL('c_gsi_handler.dll')
l = lib.process_gsi_payload
l(b'{"player": ...}')
```
...
## Best Practices
- Consider HTTP POST and JSON parsing speed if triggering real-time haptics (prefer native code for low latency).
- You can interface C with Python as a shared library (for fast processing), or keep them as separate processes linked by sockets or pipes.
- Use [Valve's sample config structure](https://developer.valvesoftware.com/wiki/Counter-Strike:_Global_Offensive_Game_State_Integration#Sample_Configuration_File) as a template for new games.

---

# Python GSI Integration Libraries for CS:GO/CS2

Several open-source Python libraries allow developers to receive and process CS:GO and CS2 Game State Integration payloads out of the box. Highly recommended for fast prototyping, telemetry pipelines, or as reference for crafting your own bridge:

---
## [`csgo-gsi-python`](https://github.com/Erlendeikeland/csgo-gsi-python)

- Simple Python server to receive CS:GO (and compatible CS2) GSI HTTP POSTs.
- MIT licensed. 
- Usage overview:

  1. Copy the provided `gamestate_integration_GSI.cfg` to your CS:GO/CS2 `csgo/cfg` directory.
  2. In Python:
     ```python
     from csgo_gsi_python import GSIServer
     server = GSIServer(('127.0.0.1', 3000), 'TOKEN_HERE')
     server.start_server()
     # Access info as:
     server.get_info('map', 'name')
     server.get_info('player', 'state', 'flashed')
     ```
- Allows for direct attribute access to nested data and easy event loop integration.
- Repo: https://github.com/Erlendeikeland/csgo-gsi-python

---
## [`CSGO-GSI`](https://github.com/mdarvanaghi/CSGO-GSI)

- Pure Python library to implement a GSI endpoint server for CS:GO telemetry.
- MIT licensed. 
- Usage overview:
  1. Copy the sample config `gamestate_integration_itulan.cfg` to your CS:GO `csgo/cfg`.
  2. Run the included server:
     ```bash
     python gsi_server.py
     ```
  3. Parse/integrate with modules: `payloadparser.py`, `player.py`, `map.py`, etc.
- Provides convenient abstractions for map/player state, logger, and payload parsing.
- Repo: https://github.com/mdarvanaghi/CSGO-GSI

---

**Tip:** Both of these libraries allow you to skip manual HTTP server setup and directly hook Python code to game state events—ideal for rapid integration, testing, or as a template for deeper C/C++ or cross-language pipelines.

See repo READMEs for configuration, token/setup, and extension details.