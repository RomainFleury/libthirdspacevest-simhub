...
class EventType(Enum):
    ...
    # Team Fortress 2 integration
    TF2_STARTED = "tf2_started"
    TF2_STOPPED = "tf2_stopped"
    TF2_GAME_EVENT = "tf2_game_event"
    # Predefined effects
...
# =============================================================================
# Team Fortress 2 Integration Protocol
# =============================================================================

def event_tf2_started(log_path: str) -> Event:
    """Event when Team Fortress 2 integration starts."""
    return Event(event=EventType.TF2_STARTED.value, log_path=log_path)


def event_tf2_stopped() -> Event:
    """Event when Team Fortress 2 integration stops."""
    return Event(event=EventType.TF2_STOPPED.value)


def event_tf2_game_event(
    event_type: str,
    params: Optional[Dict[str, Any]] = None,
) -> Event:
    """
    Team Fortress 2 game event (damage_taken, kill, backstab, etc.).
    
    Args:
        event_type: Type of event ("damage_taken", "kill", "backstab", "headshot", etc.)
        params: Event parameters (attacker, damage, victim, weapon, etc.)
    """
    return Event(
        event=EventType.TF2_GAME_EVENT.value,
        event_type=event_type,
        params=params,
    )


def response_tf2_start(
    success: bool,
    log_path: Optional[str] = None,
    error: Optional[str] = None,
    req_id: Optional[str] = None,
) -> Response:
    """Response to tf2_start command."""
    return Response(
        response="tf2_start",
        req_id=req_id,
        success=success,
...
def response_tf2_stop(
    success: bool,
    error: Optional[str] = None,
    req_id: Optional[str] = None,
) -> Response:
    """Response to tf2_stop command."""
    return Response(
        response="tf2_stop",
        req_id=req_id,
        success=success,
...
def response_tf2_status(
    ...
    """Response to tf2_status command."""
    return Response(
        response="tf2_status",
        req_id=req_id,
        running=running,
